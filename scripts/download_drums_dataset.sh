...
DATASET_DIR="${ROOT_DIR}/${DRUMS_DATASET_RELATIVE_DIR}"
DOWNLOAD_DIR="${ROOT_DIR}/data/.downloads"
ARCHIVE_PATH="${DOWNLOAD_DIR}/nerf_synthetic.zip"
...
if [[ -f "${ARCHIVE_PATH}" ]]; then
    ...
    if [[ "${actual_checksum}" != "${NERF_SYNTHETIC_DATA_SHA256}" ]]; then
...
if [[ ! -f "${ARCHIVE_PATH}" ]]; then
    echo "Downloading the official NeRF Synthetic archive (1.18 GiB)..."
    curl \
        --location \
        --fail \
        --retry 3 \
    ...
    if [[ "${actual_checksum}" != "${NERF_SYNTHETIC_DATA_SHA256}" ]]; then
        ...
        echo "expected: ${NERF_SYNTHETIC_DATA_SHA256}" >&2
...
staging_dir="$(mktemp -d "${ROOT_DIR}/data/.drums-extract.XXXXXX")"
trap 'rm -rf "${staging_dir}"' EXIT

echo "Extracting Drums poses and RGBA color images..."
unzip -q \
    "${ARCHIVE_PATH}" \
    'nerf_synthetic/drums/*' \
    -x '*_depth_*.png' '*_normal_*.png' \
    -d "${staging_dir}"

extracted_dir="${staging_dir}/nerf_synthetic/drums"
